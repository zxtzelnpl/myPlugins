...
function chart2(wrapID, title, axisX, axisY) {
    ...
    /*开始画值的区域*/
    paintArea(context, startPoint, points, endPoint, '#fef0f0');
    /*画出至之间的连线*/
    points.forEach(function (value, index) {
        var x1 = points[index - 1] ? points[index - 1].x : startPoint.x
            , y1 = points[index - 1] ? points[index - 1].y : startPoint.y
            , x2 = value.x
            , y2 = value.y
            , c = '#ff8380'
            ;
    ...
    points.forEach(function (value) {
        var innerColor='#ff8e8c'
            ,outColor='#fff'
            ,ringColor='#ff8e8c'
            ,innerR=2*dpr
            ,outerR=5*dpr
            ;
        paintCircle(context, value.x, value.y, outerR, outColor);
        paintCircle(context, value.x, value.y, innerR, innerColor);
        paintRing(context, value.x, value.y, outerR, ringColor);
    });
    ...
    /*取值开始画取值面积*/
    function paintArea(ctx, startPoint, points, endPoint, c) {
        ctx.save();
        ctx.beginPath();
        ctx.fillStyle = c;
        ctx.globalAlpha = 0.5;
